...
class Solution:
    '''
    There are n boxes labeled from 0 to n - 1. Also give are four arrays:
    status, candies, keys, and containedBoxes where:
    * status[i] is 1 if the ith box is open and 0 if the ith box is closed,
    * candies[i] is the number of candies in the ith box,
    * keys[i] is a list of the labels of the boxes that can be opened after
      opening the ith box,
    * containedBoxes[i] is a list of the boxes that are contained in the ith
      box.

    Given an integer array initialBoxes that contains the labels of the boxes
    initially obtained. It is possible to take all the candies in any open box
    and can use the keys in it to open new boxes and can use the boxes found in
    an open box.

    Return the maximum number of candies that be obtained after following the
    above rules.
    '''
    def maxCandies(self, status: List[int], candies: List[int], keys: List[List[int]], containedBoxes: List[List[int]], initialBoxes: List[int]) -> int:
        answer = 0
        opened = set()
        locked = {i for i in initialBoxes if status[i] == 0}
...
class UnitTesting(unittest.TestCase):
    def test_one(self):
        ...
        m = [0]
        o = 16
        self.assertEqual(s.maxCandies(i,j,k,l,m), o)
